...
'Payment: cash on ' + o.fulfillment,
'',
'Telegram: @' + (customer.username || '(no username)') + ' (id: ' + customer.id + ')',
].filter(Boolean).join('\n');
...
await ctx.editMessageText('Order placed! We will be in touch to confirm ' + o.fulfillment + ' details. Thanks for choosing Runtz Farm!');
ctx.session = { cart: [], stage: null, order: {} };
});
